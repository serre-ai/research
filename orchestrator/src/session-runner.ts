...
export class SessionRunner {
  ...
  /** Run a collective action session — no git worktree, uses API calls instead. */
  async runCollective(brief: SessionBrief): Promise<SessionResult> {
    const sessionId = randomUUID();
    const writer = new TranscriptWriter(this.rootDir, brief.projectName, sessionId);
    const startTime = Date.now();

    const prompt = await this.buildCollectivePrompt(brief);
    ...
    const abortController = new AbortController();
    const timeout = setTimeout(() => abortController.abort(), brief.constraints.maxDurationMs);
    ...
    try {
      ...
      for await (const message of stream) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        if ((message as any).type !== "stream_event") await writer.write(message);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        if ((message as any).type === "result") resultMessage = message;
      }
    } catch (err) {
      if (abortController.signal.aborted) {
        sessionStatus = "timeout";
    ...
    if (resultMessage) {
      if (resultMessage.subtype === "error_max_turns") { sessionStatus = "timeout"; error = "Exceeded max turns"; }
      else if (resultMessage.subtype === "error_max_budget_usd") { sessionStatus = "budget_exceeded"; error = "Exceeded budget"; }
      else if (resultMessage.subtype === "error_during_execution") { sessionStatus = "failed"; error = "Execution error"; }
    }
  ...
  private async buildCollectivePrompt(brief: SessionBrief): Promise<string> {
    ...
    sections.push(
      "# Collective Action Session\n\n" +
      "You are an autonomous agent performing a collective action for the Deepwork research platform. " +
      "Your work is done via API calls, NOT by editing files or making git commits.\n",
    );

    // Brief objective and context
    ...
    sections.push(
      ...
      "- `POST /api/forum/threads` — create thread: `{author, title, body, post_type}`\n" +
      "- `POST /api/forum/threads/:id/reply` — reply: `{author, body}`\n" +
      "- `POST /api/forum/threads/:id/vote` — vote: `{voter, position}` (support/oppose/abstain)\n\n" +
      "### Governance\n" +
      "- `GET /api/governance` — list governance decisions\n\n" +
      "### Rituals\n" +
      "- `GET /api/rituals` — list rituals\n" +
      "- `POST /api/rituals/:id/complete` — mark ritual complete: `{outcome, metadata}`\n\n" +
      "### Messages\n" +
      "- `GET /api/messages?to=<agent>` — get messages for agent\n" +
      "- `POST /api/messages` — send: `{from_agent, to_agent, subject, body, priority}`\n" +
