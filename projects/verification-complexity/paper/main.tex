...
\textbf{Part (c).}
The error correlation $\rho$ depends on the \emph{structure} of the computational bottleneck, not solely on $\VC(\calF)$.
Consider two cases.
\emph{Case 1 (shared bottleneck):} If the model lacks a specific computational capability required for all instances of a task --- e.g., an algorithm (Type~4), sufficient circuit depth (Types~2--3), or sensitivity computation (Type~1) --- then errors are systematic: the model fails on the same instances regardless of sampling randomness, producing $\rho > 0$.
\emph{Case 2 (stochastic bottleneck):} If the task's difficulty is instance-specific and stochastic --- e.g., satisfiability near the phase transition, where each random formula has unpredictable hardness --- then different sampling paths (or different models) may succeed or fail on different instances, producing $\rho \approx 0$.
For within-model self-consistency sampling at temperature $T > 0$, Case~1 dominates because all samples share the same model weights and thus the same structural limitations.
The effective sample size $N_{\text{eff}}$ is therefore bounded by the structural component of the error, independent of $N$.
\end{proof}
...
Our empirical error correlation analysis reveals a nuance not captured by naive applications of \Cref{thm:selfconsistency}(c): error correlation tracks \emph{shared structural bottlenecks} rather than worst-case verification complexity.
Algorithmic tasks (Type~4, $\VC = \PTIME$) show the highest between-model correlation ($\rho = 0.42$) because all models lack the same algorithm.
Intractability tasks (Type~5, $\VC \supseteq \coNP$) show the \emph{lowest} correlation ($\rho = 0.06$) because instance-level difficulty near the phase transition is stochastic.
This refined understanding strengthens the practical implications: self-consistency is most effective precisely where error correlation is lowest, regardless of $\VC$ class.
