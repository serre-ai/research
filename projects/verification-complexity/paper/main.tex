...
\begin{proof}
% TODO: Expand this proof sketch with full detail
Let $E_i$ be the event that sample $i$ is incorrect. Then:
\[
\Pr[E_i \cap E_j] \geq \Pr[E_i \cap E_j \mid B] \Pr[B] = r^2 q
\]
If errors were independent, $\Pr[E_i \cap E_j] = \Pr[E_i]\Pr[E_j]$. The excess correlation gives $\rho > 0$ by standard calculation.
\end{proof}
...
% TODO: Expand with complete derivation showing:
% 1. Plurality condition implies convergence (Condorcet jury)
% 2. Correlated errors reduce effective sample size (Lemma 1)
% 3. Verification hardness produces correlation (Lemma 2)
% 4. Combine to get the full statement

\subsection{Proof of Gap Collapse Sharpness}

% TODO: Prove that the gap collapse for HTN planning is tight:
% - Show that classical STRIPS planning has VC = P
% - Show that adding hierarchical decomposition strictly increases VC to coNP
% - Prove that this is the minimal increase (not PSPACE) for grounded HTN
